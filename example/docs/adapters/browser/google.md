...
- `google search` may trigger CAPTCHA in Standalone browser mode. Extension mode (with an established Chrome session) is
  more reliable.
...
- `search` output includes three result types: `result` (standard), `snippet` (featured answer box), and `paa` (People
  Also Ask).
