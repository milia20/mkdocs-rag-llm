...
AI image/video generation and editing on [yollomi.com](https://yollomi.com). Uses the same `/api/ai/*` routes as the web
app; authentication is your **logged-in Chrome session** (NextAuth cookies).
...
- **Upload**: Local paths for tools are not accepted directly — use `yollomi upload` to get a URL, or pass an existing
  HTTPS image URL.
