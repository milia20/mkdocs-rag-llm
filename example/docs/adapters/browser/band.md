...
Read posts, comments, and notifications from [Band](https://www.band.us), a private community platform. Authentication
uses your logged-in Chrome session (cookie-based).
...
- `band post` output rows: `type=post` (the post itself), `type=comment` (top-level comment), `type=reply` (nested
  reply)
